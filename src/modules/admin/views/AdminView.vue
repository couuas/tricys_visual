...
const updateCharts = () => {
  ...
  const stats = systemStats.value.stats;
  ...
  charts.user.setOption({
    ...
    series: [{
      ...
      data: [
        { value: stats.users.admin, name: 'Admin', itemStyle: { color: '#a855f7' } },
        { value: stats.users.operator, name: 'Operator', itemStyle: { color: '#00d2ff' } },
        { value: stats.users.total - stats.users.active, name: 'Inactive', itemStyle: { color: '#30363d' } }
      ]
    }],
  ...
  charts.project.setOption({
    ...
    series: [{
      ...
      barWidth: '60%',
      data: [
        { value: stats.projects.public, itemStyle: { color: '#00d2ff' } },
        { value: stats.projects.private, itemStyle: { color: '#238636' } }
      ]
    }],
    backgroundColor: 'transparent'
  });
  ...
  charts.goview.setOption({
    ...
    series: [{
      ...
      label: { show: false },
      data: [
        { value: stats.goview.published, name: 'Published', itemStyle: { color: '#00d2ff' } },
        { value: stats.goview.draft, name: 'Draft', itemStyle: { color: '#888' } }
      ]
    }],
    backgroundColor: 'transparent'
  });
...
const fetchLogs = async () => {
  try {
    ...
    if (logContainer.value) {
      setTimeout(() => { logContainer.value.scrollTop = logContainer.value.scrollHeight; }, 50);
    }
  } catch (e) {
  }
};
...
const downloadLogs = async () => {
  try {
    ...
    link.href = url;
    link.setAttribute('download', 'backend_log.txt');
    document.body.appendChild(link);
    link.click();
    link.remove();
  } catch (e) {
    $notify({ title: 'Error', message: 'Download failed.', type: 'error' });
  }
};
...
const togglePublish = async (project, isPublic) => {
  try {
    const endpoint = isPublic ? 'publish' : 'unpublish';
    await apiClient.patch(`/admin/projects/${project.id}/${endpoint}`);
...
const deleteAsset = async (model) => {
  if (!confirm(`Delete Model ${model.name}?`)) return;
  try {
    await apiClient.delete(`/admin/assets/models/${model.name}`);
    fetchAssets();
  } catch {
    $notify({ title: 'Failed', type: 'error' });
  }
...
const deleteGoviewProject = async (project) => {
  if (!confirm(`Delete GoView Project ${project.project_name}?`)) return;
  try {
    await apiClient.delete(`/admin/goview/projects/${project.id}`);
    fetchGoviewProjects();
    $notify({ title: 'Deleted', type: 'success' });
  } catch {
    $notify({ title: 'Failed', type: 'error' });
...
const copyLink = (project) => {
  const url = `${window.location.origin}/goview/#/chart/preview/${project.id}`;
...
const uploadAssetModel = async (event) => {
  const file = event.target.files[0];
  if (!file) return;
  const formData = new FormData();
  formData.append('file', file);
  try {
    await apiClient.post('/admin/assets/models', formData);
    fetchAssets();
    $notify({ title: 'Uploaded', type: 'success' });
  } catch {
    $notify({ title: 'Failed', type: 'error' });
  }
};
const updateDemoTemplate = async (event) => {
  const file = event.target.files[0];
  if (!file) return;
  const formData = new FormData();
  formData.append('file', file);
  try {
    await apiClient.post('/admin/assets/example_model', formData);
    $notify({ title: 'Template Updated', type: 'success' });
  } catch {
    $notify({ title: 'Failed', type: 'error' });
  }
};
