      ...
      >
        ...
        <div class="section-list" @click.self="cancelRenameIfEditing">
          ...
          <div v-if="viewMode === 'personal'" class="project-list custom-scroll" @click.self="cancelRenameIfEditing">
            <div v-for="project in recentProjects" :key="project.id || project.project_id" class="project-row" :class="{ editing: isProjectEditing(project), active: isProjectSelected(project) }" @click="handleProjectRowClick(project)">
              ...
              <div class="p-actions">
                ...
                <template v-else>
                  ...
                  <button class="btn-jump alt" @click.stop="openExhibition(project)" :disabled="isAnyProcessing">EX</button>
            ...
            <div class="project-init-hint" aria-label="Project initialization hint">
              <span class="hint-arrow">←</span>
              <div class="hint-copy">
                <div class="hint-title">Initialize a project from the left panel</div>
                <div class="hint-desc">Upload source, import archive, or launch the system demo to create a workspace.</div>
              </div>
            </div>
          </div>

          <div v-else class="project-list custom-scroll">
            <div v-for="project in publicProjects" :key="project.id" class="project-row" :class="{ active: isProjectSelected(project) }" @click="previewProject(project)">
              <div class="row-icon">🌍</div>
              <div class="p-info">
        ...
        <template #right>
          <aside class="section-inspector">
            <div class="inspector-panel summary-panel">
              <h3>WORKSPACE SUMMARY</h3>
              <div class="summary-grid">
                <div class="summary-item">
                  <span class="summary-value">{{ projectStats.personal }}</span>
                  <span class="summary-label">Personal</span>
                </div>
                <div class="summary-item">
                  <span class="summary-value">{{ projectStats.public }}</span>
                  <span class="summary-label">Public</span>
                </div>
                <div class="summary-item wide">
                  <span class="summary-value small">{{ lastProjectId || 'None' }}</span>
                  <span class="summary-label">Last Active</span>
                </div>
              </div>
            </div>

            <div class="inspector-panel detail-panel">
              <h3>PROJECT INSPECTOR</h3>
              <template v-if="selectedProject">
                <div class="detail-title">{{ selectedProject.name || 'Untitled Project' }}</div>
                <div class="detail-row">
                  <span>ID</span>
                  <strong>{{ getProjectId(selectedProject) || '--' }}</strong>
                </div>
                <div class="detail-row">
                  <span>Created</span>
                ...
                <div class="detail-row">
                  <span>Source</span>
                  <strong>{{ isPublicProject(selectedProject) ? 'Public' : 'Personal' }}</strong>
                </div>
                <div class="inspector-actions">
                  <button class="panel-action primary" @click="openWorkspace(selectedProject)">Open Workspace</button>
                  <button class="panel-action" @click="openExhibition(selectedProject)">Open Exhibition</button>
                  <button class="panel-action" @click="handleExport(selectedProject)">Export Archive</button>
                  <button
                    v-if="isPublicProject(selectedProject)"
                    class="panel-action"
                    @click="handleFork(getProjectId(selectedProject))"
                    :disabled="isAnyProcessing"
                  >Fork Project</button>
                </div>
              </template>
              <div v-else class="inspector-empty">
                Select a project row to inspect metadata and actions.
              </div>
            </div>
          </aside>
        </template>
...
import { useProjectsWorkspace } from '../composables/useProjectsWorkspace';
import ProjectsWorkbenchShell from '../components/ProjectsWorkbenchShell.vue';
...
const projectStats = computed(() => ({
  personal: recentProjects.value.length,
  public: publicProjects.value.length
}));

const activeProjectList = computed(() => viewMode.value === 'personal' ? recentProjects.value : publicProjects.value);

const selectedProject = computed(() => {
  if (!selectedProjectId.value) {
    return activeProjectList.value[0] || null;
  }
  return [...recentProjects.value, ...publicProjects.value].find((project) => getProjectId(project) === selectedProjectId.value) || null;
});

const isProjectEditing = (project) => getProjectId(project) === editingProjectId.value;

const isProjectSelected = (project) => getProjectId(project) === selectedProjectId.value;

const isPublicProject = (project) => {
  const projectId = getProjectId(project);
  return Boolean(projectId && publicProjects.value.some((item) => getProjectId(item) === projectId));
};
...
.section-create { width: 100%; height: 100%; min-width: 0; padding: 30px; background: linear-gradient(135deg, rgba(0,0,0,0.2), transparent); overflow-y: auto; display: flex; flex-direction: column; min-height: 0; box-sizing: border-box; }
...
.section-list { width: 100%; height: 100%; min-width: 0; padding: 30px; background: #0b0e14; display: flex; flex-direction: column; overflow: hidden; box-sizing: border-box; }
.section-inspector { width: 100%; height: 100%; min-width: 0; padding: 24px; box-sizing: border-box; display: flex; flex-direction: column; gap: 16px; overflow-y: auto; background: #090c12; }
...
.project-row.active { border-color: rgba(0, 210, 255, 0.6); background: rgba(0, 210, 255, 0.08); box-shadow: inset 3px 0 0 #00d2ff; }
...
.project-init-hint { flex-shrink: 0; margin-top: 4px; padding: 14px 16px; border: 1px dashed rgba(0, 210, 255, 0.28); border-radius: 6px; background: rgba(0, 210, 255, 0.035); color: #8b949e; display: flex; align-items: center; gap: 14px; }
.hint-arrow { flex: 0 0 auto; color: #00d2ff; font-size: 24px; line-height: 1; font-weight: 800; }
.hint-copy { min-width: 0; }
.hint-title { color: #dbe8f5; font-size: 12px; font-weight: 800; letter-spacing: 0.4px; text-transform: uppercase; }
.hint-desc { margin-top: 4px; color: #64748b; font-size: 11px; line-height: 1.45; }
...
.inspector-panel { border: 1px solid #243041; border-radius: 8px; background: rgba(13, 17, 23, 0.86); padding: 16px; min-width: 0; }
.inspector-panel h3 { margin: 0 0 14px; color: #7d8ea3; font-size: 11px; letter-spacing: 1px; text-transform: uppercase; }
.summary-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.summary-item { min-width: 0; min-height: 72px; border: 1px solid rgba(255,255,255,0.07); border-radius: 6px; background: rgba(5, 7, 10, 0.45); padding: 12px; display: flex; flex-direction: column; justify-content: center; gap: 6px; }
.summary-item.wide { grid-column: 1 / -1; }
.summary-value { color: #fff; font-family: 'JetBrains Mono', monospace; font-size: 22px; font-weight: 800; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.summary-value.small { font-size: 12px; color: #cbd5e1; }
.summary-label { color: #64748b; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; }
.detail-panel { flex: 1 1 auto; display: flex; flex-direction: column; min-height: 240px; }
.detail-title { color: #f8fafc; font-size: 16px; font-weight: 800; line-height: 1.3; margin-bottom: 14px; overflow-wrap: anywhere; }
.detail-row { display: flex; flex-direction: column; gap: 5px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); }
.detail-row span { color: #64748b; font-size: 10px; text-transform: uppercase; letter-spacing: 0.8px; font-weight: 700; }
.detail-row strong { color: #dbe8f5; font-size: 12px; font-family: 'JetBrains Mono', monospace; font-weight: 600; overflow-wrap: anywhere; }
.inspector-actions { display: grid; grid-template-columns: 1fr; gap: 10px; margin-top: auto; padding-top: 18px; }
.panel-action { min-height: 38px; border-radius: 6px; border: 1px solid #334155; background: #0f172a; color: #dbe8f5; cursor: pointer; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.4px; transition: 0.2s; }
.panel-action:hover:not(:disabled) { border-color: #00d2ff; color: #00d2ff; background: rgba(0, 210, 255, 0.06); }
.panel-action.primary { border-color: transparent; background: #00d2ff; color: #03131a; }
.panel-action.primary:hover:not(:disabled) { color: #03131a; filter: brightness(1.08); }
.panel-action:disabled { opacity: 0.55; cursor: not-allowed; }
.inspector-empty { flex: 1; border: 1px dashed #30363d; border-radius: 6px; color: #64748b; display: flex; align-items: center; justify-content: center; padding: 18px; text-align: center; font-size: 12px; line-height: 1.5; background: rgba(255,255,255,0.01); }
