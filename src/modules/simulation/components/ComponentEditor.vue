<template>
  <div class="editor-panel" :class="{ open: selectedId, 'embedded-mode': embedded }" @click.stop>
          ...
          <div class="model-selector" ref="modelSelectorRef">
            ...
            <transition name="dropdown-fade">
              <div v-if="showModelDropdown" class="custom-dropdown-menu">
                ...
                <div v-for="model in libraryModels" :key="model.url" class="dropdown-item" :class="{ selected: selectedModelUrl === model.url }" @click="selectModel(model.url)"><span class="item-icon">📦</span> {{ model.name }}</div>
    ...
    <div v-if="selectedId" class="actions">
      ...
      <button class="btn secondary" @click="resetChanges" :disabled="!hasChanges" :title="!hasChanges ? 'No changes to reset' : 'Revert to last saved state'">
        Reset
      </button>
    </div>
...
import { useSimulationLibraryModels } from '../composables/useSimulationLibraryModels';
...
const {
  ...
  currentProjectId, isReadOnly, structureData,
} = useProjectWorkspace();

const {
  libraryModels, fetchLibraryModels,
} = useSimulationLibraryModels();

const { hasSimulationData } = useSimulationSessionStore();

const { filterSchema, saveComponentFilterRule } = useSimulationWorkspace();
const { isGroup, isExpanded, setExpandedGroup, dissolveGroup, componentGroups } = useSimulationSceneStore({ currentProjectId, modelConfig, structureData });
    ...
    if (componentGroups.value && componentGroups.value[safeId]) {
      ...
      if (groupDef && groupDef.children) {
        ...
        groupDef.children.forEach((childId) => {
          ...
          const childParams = allParams.filter((param) => param.name.startsWith(prefix));
          childParams.forEach((param) => {
            bufferList.push(formatArrayParam(param));
          });
        });
      }
    }
  } else {
    const filteredParams = allParams.filter((param) => param.name.toLowerCase().startsWith(safeIdLower + '.'));
...
watch(
  [() => props.selectedId, componentParams, componentGroups, modelConfig, filterSchema],
  loadEditorData,
...
const getChildParams = (childId) => {
  ...
  return localParamsList.value.filter((param) => param.name.toLowerCase().startsWith(prefix));
};

const getParamName = (fullName) => fullName.split('.').slice(1).join('.');
const getParamOwner = (fullName) => fullName.split('.')[0];
...
const getSimpleName = (fullName) => fullName.split('.').pop();

const isParamModified = (name) => {
  const current = localParamsList.value.find((param) => param.name === name);
  const orig = originalParams.value.find((param) => param.name === name);
  if (!current || !orig) return false;
  return current.value != orig.value;
};

const revertToDefault = (name) => {
  const currentParam = localParamsList.value.find((param) => param.name === name);
  if (currentParam) {
...
function checkUserInputValue(str) {
  ...
  if (prefixes.some((prefix) => s.toLowerCase().startsWith(prefix))) return;
...
const saveAll = async () => {
  ...
  if (!hasSimulationData.value) {
    try {
      localParamsList.value.forEach((param) => {
        if (param.value !== undefined && param.value !== null) {
          checkUserInputValue(String(param.value));
        }
      });
    } catch (err) {
  ...
  try {
    ...
    if (!hasSimulationData.value) {
      if (Array.isArray(componentParams.value)) {
        localParamsList.value.forEach((localParam) => {
          const masterParam = componentParams.value.find((param) => param.name === localParam.name);
          if (masterParam) masterParam.value = localParam.value;
        });
      }
...
const currentModelName = computed(() => {
  ...
  const found = libraryModels.value.find((model) => model.url === selectedModelUrl.value);
