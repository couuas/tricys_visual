...
const props = defineProps({
    ...
    getConnectionStyle: { type: Function, default: () => ({ width: 4.0, type: 'flow', color: 0x00d2ff, speed: 0.04, opacity: 0.8 }) },
...
// Ensure we hold the engine without Deep Reactive proxying
const engine = shallowRef(null);

onMounted(async () => {
    if (!viewportRef.value) return;

    // Create the isolated engine instance
    ...
    // Provide generic history callbacks for bridging to the vue store
    ...
    instance.on('dragEnded', (payload) => {
        // e.g. payload: { id, deltaX, deltaY, finalX, finalY }
...
watch(() => props.structureData, (newVal) => {
    if (engine.value && newVal && !isBoxSelecting.value) { // Avoid rebuild during box select UI loop
