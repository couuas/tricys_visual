    ...
    <div class="snapshot-panel">
        <div class="snapshot-panel-header">
            ...
            <div v-show="historyStackLength > 0" class="history-controls">
                <button class="history-btn" :disabled="historyCursor <= 0 || isReadOnly" @click="$emit('undo')" title="Undo">↶</button>
                <span class="history-status">{{ historyCursor }} / {{ Math.max(historyStackLength - 1, 0) }}</span>
                <button class="history-btn" :disabled="historyCursor >= historyStackLength - 1 || isReadOnly" @click="$emit('redo')" title="Redo">↷</button>
            </div>
        </div>

        <div class="snapshot-panel-body">
            <div class="snapshot-actions">
                <button class="history-btn" :disabled="isReadOnly" @click="$emit('save')">Save</button>
                <button class="history-btn" :disabled="!modelValue || isReadOnly" @click="$emit('restore')">Restore</button>
                <button class="history-btn" :disabled="!modelValue || isReadOnly" @click="$emit('delete')">Delete</button>
            </div>
